...
def load_churn_data():
    ...
    try:
        ...
        date_columns = ['signup_date', 'last_transaction_date', 'last_activity_date', 'feature_created_at']
...
@app.get("/churn-features", response_model=List[ChurnFeatureResponse], tags=["Features"])
async def get_churn_features(
    limit: int = Query(100, ge=1, le=10000, description="Number of records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    churn_flag: Optional[int] = Query(None, description="Filter by churn flag (0 or 1)"),
    region: Optional[str] = Query(None, description="Filter by region"),
    channel: Optional[str] = Query(None, description="Filter by channel"),
    lifecycle_stage: Optional[str] = Query(None, description="Filter by user lifecycle stage"),
    min_spend: Optional[float] = Query(None, ge=0, description="Minimum total spend"),
    max_days_inactive: Optional[int] = Query(None, ge=0, description="Maximum days since last activity")
):
...
async def get_dataset_stats():
    ...
    try:
        ...
        top_channels = [{"channel": k, "count": int(v)} for k, v in channel_stats.items()]
    ...
    try:
        ...
        if format.lower() == "csv":
            ...
            return StreamingResponse(
                ...
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename=churn_features{suffix}_{timestamp}.csv"}
            )
            ...
            return StreamingResponse(
                ...
                headers={"Content-Disposition": f"attachment; filename=churn_features{suffix}_{timestamp}.json"}
            )
...
@app.get("/churn-features/user/{user_id}", response_model=ChurnFeatureResponse, tags=["Features"])
...
async def get_user_segments():
    ...
    try:
        ...
        # RFM segments
        rfm_segments = df.groupby(['recency_score', 'frequency_score', 'monetary_score']).agg({
            'user_id': 'count',
            'churn_flag': 'mean',
            'total_spend_ngn': 'mean'
        }).reset_index()
        
        rfm_segments.columns = ['recency', 'frequency', 'monetary', 'user_count', 'churn_rate', 'avg_spend']
        rfm_segments['churn_rate'] = rfm_segments['churn_rate'].round(3)
        rfm_segments['avg_spend'] = rfm_segments['avg_spend'].round(2)
        
        # Lifecycle segments
        lifecycle_segments = df.groupby('user_lifecycle_stage').agg({
            'user_id': 'count',
            'churn_flag': 'mean',
            'total_spend_ngn': 'mean',
        ...
        lifecycle_segments.columns = ['lifecycle_stage', 'user_count', 'churn_rate', 'avg_spend', 'avg_sessions']
