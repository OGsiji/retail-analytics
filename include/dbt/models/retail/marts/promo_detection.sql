{{
  config(
    ...
    schema='retail_marts',
    post_hook=[
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_bidco ON {{ this }}(is_bidco)",
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_promo ON {{ this }}(is_on_promo)",
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_store ON {{ this }}(store_name)",
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_supplier ON {{ this }}(supplier)",
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_composite ON {{ this }}(is_bidco, is_on_promo)",
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_item ON {{ this }}(item_code)",
      "CREATE INDEX IF NOT EXISTS idx_promo_detection_dates ON {{ this }}(first_sale_date, last_sale_date)"
    ]
  )
}}
...
promo_classification AS (
    SELECT
        ...
        CASE
            WHEN dss.discount_from_rrp_pct >= ({{ var('promo_discount_threshold') }} * 100)
            THEN 1
            ELSE 0
        END as is_on_promo,
