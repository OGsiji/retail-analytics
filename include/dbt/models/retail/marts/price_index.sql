...
sku_with_benchmarks AS (
    SELECT
        ...
        {{ calculate_price_index('ssp.avg_realized_price', 'osp.overall_section_avg_price') }} as price_index_vs_competitors,
        -- Calculate RRP index
        {{ calculate_price_index('ssp.avg_rrp', 'osp.overall_section_avg_rrp') }} as rrp_index_vs_competitors,
...
with_positioning AS (
    SELECT
        *,
        {{ categorize_price_position('price_index_vs_competitors') }} as price_positioning,
        CASE
