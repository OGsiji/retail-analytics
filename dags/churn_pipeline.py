...
try:
    ...
    DBT_AVAILABLE = False
    logging.warning(
        "dbt/Cosmos not available. Transformation tasks will use plain SQL."
    )
...
def churn_prediction_pipeline():
    ...
    **Output:**
    ...
    def load_transaction_dump():
        ...
        try:
            with open(
                "/usr/local/airflow/include/datasets/postgres_transactions_dump.sql",
                "r",
            ) as file:
            ...
            row_count = postgres_hook.get_first("SELECT COUNT(*) FROM transactions;")[0]
    ...
    def validate_sql_dump_data():
        ...
        try:
            # Check users table
            users_count = postgres_hook.get_first("SELECT COUNT(*) FROM users;")[0]
    ...
    def load_activities_csv():
        ...
        try:
            ...
            # Clear existing data
            postgres_hook.run(
                "TRUNCATE TABLE churn_analytics.raw_user_activities CASCADE;"
            )
    ...
    def validate_raw_data(activities_result, sql_dump_result):
        ...
        try:
            ...
            if unmatched_activities > 0:
                logging.warning(
                    f"Found {unmatched_activities} activities for non-existent users"
                )
    ...
    def refresh_api_cache():
        ...
        try:
            ...
            response = requests.post("http://churn-api:8000/refresh-cache", timeout=30)
            ...
            if response.status_code == 200:
                ...
                logging.warning(
                    f"API cache refresh returned status: {response.status_code}"
                )
